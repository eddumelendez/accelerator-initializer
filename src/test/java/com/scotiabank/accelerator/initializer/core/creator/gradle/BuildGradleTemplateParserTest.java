...
public class BuildGradleTemplateParserTest {
	...
	private Map<String, Object> createArgs() {
		return ImmutableMap.<String, Object>builder().put("ACCP_VERSION", "1.2.1")
				.put("SPRING_BOOT_VERSION", "1.5.2.RELEASE").put("IS_SPRING_BOOT", true)
				.put("REPOSITORY_NAME", "initializer").put("GROUP_ID", "com.bns.hopper")
				.build();
	}
	...
	public void assertBuildGradleIsCalled() {
		...
		verify(this.fileProcessor, times(1)).processTemplate(
				BuildGradleTemplateParser.BUILDSCRIPT_PATH, createArgs());
	...
	public void asserAddApplyPluginIsCalled() {
		...
		verify(this.fileProcessor, times(1)).processTemplate(
				BuildGradleTemplateParser.APPLY_PLUGINS_PATH, createArgs());
		verify(this.fileProcessor, times(1)).writeContentTo(this.file, null);
	}
	...
	public void asserAddDependenciesIsCalled() {
		...
		verify(this.fileProcessor, times(1)).processTemplate(
				BuildGradleTemplateParser.DEPENDENCIES_PATH, createArgs());
		verify(this.fileProcessor, times(1)).writeContentTo(this.file, null);
	}
	...
	public void asserAddJarIsCalled() {
		this.parser.addJar();
		verify(this.fileProcessor, times(1)).processTemplate(
				BuildGradleTemplateParser.JAR_PATH, Collections.emptyMap());
		verify(this.fileProcessor, times(1)).writeContentTo(this.file, null);
	}
	...
	public void asserAddIntegrationTestIsCalled() {
		...
		verify(this.fileProcessor, times(1)).processTemplate(
				BuildGradleTemplateParser.INTEGRATION_TEST_PATH, Collections.emptyMap());
		verify(this.fileProcessor, times(1)).writeContentTo(this.file, null);
	}
	...
	public void asserAddProjectMetadatatIsCalled() {
		...
		verify(this.fileProcessor, times(1)).processTemplate(
				BuildGradleTemplateParser.PROJECT_METADATA_PATH, createArgs());
		verify(this.fileProcessor, times(1)).writeContentTo(this.file, null);
	}

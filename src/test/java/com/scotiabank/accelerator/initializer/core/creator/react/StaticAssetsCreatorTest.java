...
public class StaticAssetsCreatorTest {
	...
	public void assertItLoadsTemplates() {
		...
		verify(this.fileProcessor, times(1))
				.loadResourceFromClassPath(StaticAssetsCreator.FAVICON_ICO_PATH);
		verify(this.fileProcessor, times(1))
				.loadResourceFromClassPath(StaticAssetsCreator.LOGO_SVG_PATH);
		verify(this.fileProcessor, times(1))
				.loadResourceFromClassPath(StaticAssetsCreator.INDEX_CSS_TPL_PATH);
		verify(this.fileProcessor, times(1))
				.loadResourceFromClassPath(StaticAssetsCreator.APP_CSS_TPL_PATH);
	}
	...
	public void assertItCreateFiles() {
		...
		verify(this.fileProcessor, times(1))
				.touch(Paths.get("./static/images/favicon.ico"));
	...
	public void assertItCopiesContentToFile() throws IOException {
		...
		when(this.fileProcessor
				.loadResourceFromClassPath(StaticAssetsCreator.APP_CSS_TPL_PATH))
						.thenReturn(new FileInputStream(appCss));
		when(this.fileProcessor
				.loadResourceFromClassPath(StaticAssetsCreator.INDEX_CSS_TPL_PATH))
						.thenReturn(new FileInputStream(indexCss));
		when(this.fileProcessor
				.loadResourceFromClassPath(StaticAssetsCreator.FAVICON_ICO_PATH))
						.thenReturn(new FileInputStream(faviconIco));
		when(this.fileProcessor
				.loadResourceFromClassPath(StaticAssetsCreator.LOGO_SVG_PATH))
						.thenReturn(new FileInputStream(logoSvg));

		when(this.fileProcessor.touch(Paths.get("./static/css/App.css")))
				.thenReturn(appCss);
		when(this.fileProcessor.touch(Paths.get("./static/css/index.css")))
				.thenReturn(indexCss);
		when(this.fileProcessor.touch(Paths.get("./static/images/logo.svg")))
				.thenReturn(logoSvg);
		when(this.fileProcessor.touch(Paths.get("./static/images/favicon.ico")))
				.thenReturn(faviconIco);

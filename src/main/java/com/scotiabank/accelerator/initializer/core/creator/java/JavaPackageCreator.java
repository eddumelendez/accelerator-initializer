...
class JavaPackageCreator implements FileCreator<ProjectCreation> {
	...
	public void create(ProjectCreation request) {
		...
		String packagePath = String.format(CreatorConstants.PACKAGE_PATH,
				request.getGroup().toLowerCase());
		Path packages = Paths.get(request.getRootDir(),
				CreatorConstants.SRC_MAIN_JAVA_PATH, packagePath);
		Path testSrc = Paths.get(request.getRootDir(),
				CreatorConstants.SRC_TEST_JAVA_PATH, packagePath);
